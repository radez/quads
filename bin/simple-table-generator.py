...
def get_cell_color(a, b, c):
    return "#" + ('0' if len(hex(int(a))) < 4 else '') + hex(int(a))[2:] + ('0' if len(hex(int(b))) < 4 else '') + hex(
        int(b))[2:] + ('0' if len(hex(int(c))) < 4 else '') + hex(int(c))[2:]
...
def print_simple_table(data, data_colors, _days):
    ...
    print("<tr>")
    print("<th>Name</th>")
    for _day in range(0, _days):
        print("<th width=20>" + ('0' if _day < 9 else '') + str(_day + 1) + "</th>")
    print("</tr>")
    for item in range(0, len(data)):
        print("<tr>")
        print("<td>" + str(data[item][0]) + "</td>")
        for j in range(0, _days):
            chosen_color = data_colors[item][j]
...
for h in your_list:
    ...
    for d in range(0, days):
        ...
        if day < 10:
            ...
            day_string = str(day)
        default, current, override = quads.find_current(h[0], "{}-{}-{} 00:00".format(year, month, day_string))
        if current:
